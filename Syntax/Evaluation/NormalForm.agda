...
mutual
  ...
  data Ne : Term → Set where
    ...
    neApp : ∀{t s} → Ne t → Nf s
          → Tm 0 t → Tm 0 s → Ne (t · s)
    neApp₁ : ∀{t s} → Nf t → Nf s → ¬Tm 0 t → Ne (t · s)
    neApp₂ : ∀{t s} → Nf t → Nf s → Tm 0 t → ¬Tm 0 s → Ne (t · s)

    neRec : ∀{z s t} → Nf z → Nf s → Ne t
          → Tm 0 z → Tm 0 s → Tm 0 t → Ne (Rec z s t)
    neRec₁ : ∀{z s t} → Nf z → Nf s → Nf t → ¬Tm 0 z → Ne (Rec z s t)
    neRec₂ : ∀{z s t} → Nf z → Nf s → Nf t → Tm 0 z → ¬Tm 0 s → Ne (Rec z s t)
...
inj-neApp₂ : ∀{t s} → Nf t → Nf s → ¬Tm 0 s → Ne (t · s)
inj-neApp₂ {t} {s} nft nfs tm with decTm 0 t
...
inj-neRec {z} {s} {t} nfz nfs ne with decTm 0 z | decTm 0 s | decTm 0 t
...
inj-neRec₂ : ∀{z s t} → Nf z → Nf s → Nf t → ¬Tm 0 s → Ne (Rec z s t)
inj-neRec₂ {z} nfz nfs nft tm with decTm 0 z
inj-neRec₂ nfz nfs nft tm | inj₁ x = neRec₂ nfz nfs nft x tm
inj-neRec₂ nfz nfs nft tm | inj₂ y = neRec₁ nfz nfs nft y

inj-neRec₃ : ∀{z s t} → Nf z → Nf s → Nf t → ¬Tm 0 t → Ne (Rec z s t)
inj-neRec₃ {z} {s} nfz nfs nft tm with decTm 0 z | decTm 0 s
...
mutual
  ...
  neWkLemma (neApp₁ x y z) = neApp₁ (nfWkLemma x) (nfWkLemma y) (sub¬Tm-lemma z)
  neWkLemma (neApp₂ x y z r) =
    neApp₂ (nfWkLemma x) (nfWkLemma y) (tm-wk-0 z) (sub¬Tm-lemma r)
  ...
  neWkLemma (neRec₁ z s t tm) =
    neRec₁ (nfWkLemma z) (nfWkLemma s) (nfWkLemma t) (sub¬Tm-lemma tm)
  neWkLemma (neRec₂ z s t tm tm') =
    neRec₂ (nfWkLemma z) (nfWkLemma s) (nfWkLemma t)
      (tm-wk-0 tm) (sub¬Tm-lemma tm')
  neWkLemma (neRec₃ z s t tmz tms tmt) =
    neRec₃ (nfWkLemma z) (nfWkLemma s) (nfWkLemma t)
      (tm-wk-0 tmz) (tm-wk-0 tms) (sub¬Tm-lemma tmt)
...
neBeta : ∀{t s} → Ne (Lam t · s) → Tm 1 t → Tm 0 s → ⊥
neBeta (neApp () x x₁ x₂) tmt tms
neBeta (neApp₁ x x₁ x₂) tmt tms = ¬Tm-lemma x₂ (tmLam tmt)
neBeta (neApp₂ x x₁ x₂ x₃) tmt tms = ¬Tm-lemma x₃ tms
