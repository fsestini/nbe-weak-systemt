...
-- was corr
sngl-msub : ∀ {s} n → msub (Free n) [ n ↦ s ] ≡ s
sngl-msub zero = refl
...
null-msub-var : ∀ {s} n x → x ≤ n → msub (Free x) [ suc n ↦ s ] ≡ Free x
null-msub-var n x p with x ≟ msLen [ n ↦ Free n ]
null-msub-var n x p | yes q rewrite q | sym ([↦]len {Free n} n) =
  ⊥-elim (absurde _ _ p (≤refl n))
null-msub-var n x p | no ¬p = null-sngl-msub x n p
